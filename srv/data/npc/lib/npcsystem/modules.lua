...
if(Modules == nil) then
	...
	function StdModule.promotePlayer(cid, message, keywords, parameters, node)
		...
		if(isPremium(cid) or not getBooleanFromString(getConfigValue('premiumForPromotion'))) then
			...
			elseif(getPlayerLevel(cid) < parameters.level) then
				npcHandler:say('I am sorry, but I can only promote you once you have reached level ' .. parameters.level .. '.', cid) --sefodeu
	...
	function StdModule.learnSpell(cid, message, keywords, parameters, node)
		...
		if(isPremium(cid) or not(parameters.premium)) then
			...
			elseif(getPlayerLevel(cid) < parameters.level) then
				npcHandler:say('You need to obtain a level of ' .. parameters.level .. ' or higher to be able to learn ' .. parameters.spellName .. '.', cid) --sefodeu
	...
	function TravelModule.travel(cid, message, keywords, parameters, node)
		...
		module.npcHandler:say('Do you want to travel to ' .. keywords[1] .. ' for ' .. parameters.cost .. ' gold coins?', cid)  --sefodeu
		return true
	end
