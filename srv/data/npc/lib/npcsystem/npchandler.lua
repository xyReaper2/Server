...
---------------------------- Saga System by Jean --------------------
local SAGA = SAGASYSTEM.NPCS[getNpcName()]
	if SAGA then	
	local failed = true
	for i=1, #SAGA do
			if getPlayerSaga(cid, SAGA[i][1], SAGA[i][2]) then
			failed = false
				if SAGA[i][3] then
    			msg = self:parseMessage(SAGA[i][3]:format(getCreatureName(cid)), parseInfo)
				end
				break
 		    end
    end	
	...
	if failed then
	msg = self:parseMessage("[SAGA SYSTEM] Não posso falar com você neste momento", parseInfo)
	end							
	end
						
------------- Mercado Negro by Jean	 --------------	
					
if getNpcName() == "Mercador" then
					local quantidade = getStorage(26000)
					if quantidade > 0 then
					local produto = getItemNameById(getStorage(26001))
					msg = "Olá "..getPlayerName(cid).." no momento possuo "..quantidade.." unidades de "..produto.." no meu estoque, está enteressado?{yes} or {no}"
					else
					msg = "Desculpe-me, estoque esgotado! Domingo às 00:00 renovarei o estoque."					
					end
					end
-------------------FIM -----------------------------
