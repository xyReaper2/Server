...
setCombatFormula(combat, COMBAT_FORMULA_LEVELMAGIC, -62, 1, -68, 1)
...
local position5 = {x=getThingPosition(getCreatureTarget(cid)).x+2, y=getThingPosition(getCreatureTarget(cid)).y, z=getThingPosition(getCreatureTarget(cid)).z}
doSendMagicEffect(position1, 587)
doSendMagicEffect(position2, 587)
doSendMagicEffect(position3, 801)
doSendMagicEffect(position4, 801)
doSendMagicEffect(position5, 801)
