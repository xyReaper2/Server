...
local effect = {579} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
setConditionParam(condition, CONDITION_PARAM_SKILL_DISTANCE, skillpoint+5)
...
function buffdanzou(tempo2,tempo3,cid)
...
if getPlayerStorageValue(cid, 102053) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    while (tempo2 ~= (tempo*1000)) do
        addEvent(buffdanzou, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 600
    end
    ...
    doCreatureSay(cid, "+DIST", TALKTYPE_MONSTER)
