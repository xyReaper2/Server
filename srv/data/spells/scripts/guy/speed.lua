...
local position127 = {x=getPlayerPosition(cid).x+0, y=getPlayerPosition(cid).y+0, z=getPlayerPosition(cid).z}
...
poslook.stackpos = STACKPOS_TOP_MOVEABLE_ITEM_OR_CREATURE
	if isWalkable(poslook, false, false, pzprotect) then
		if not isCreature(getThingfromPos(poslook).uid) then
			...
			doSendMagicEffect(position127, 320)
		return true
		end
	end
end
...
doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "Speed dash está em cooldown, será possível usá-lo novamente em ["..exhaustion.get(cid, 61590).."] segundos")
return false
end
