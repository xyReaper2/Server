...
local effect = {704} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
function buffkurenai(tempo2,tempo3,cid)
...
if getPlayerStorageValue(cid, 102053) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    while (tempo2 ~= (tempo*1000)) do
        addEvent(buffkurenai, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 600
    end
    setPlayerStorageValue(cid, 102053,1) -- storage verifica transformado, quando = 1 player esta transformado.
    doCreatureSay(cid, "Anshosui Kashosai", TALKTYPE_MONSTER)
    doSendMagicEffect(position129, 704)
else
