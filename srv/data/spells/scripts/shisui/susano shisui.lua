...
local effect = {532} -- effect no player, caso queira apenas 1, basta remover os outros numeros.
...
function buffshisui(tempo2,tempo3,cid)
...
if getPlayerStorageValue(cid, 102053) ~= 1 or getCreatureCondition(cid, CONDITION_REGENERATION, 1) == false then
    ...
    while (tempo2 ~= (tempo*1000)) do
        addEvent(buffshisui, tempo2, tempo2, tempo*1000, cid)
        tempo2 = tempo2 + 300
    end
    ...
    doSendMagicEffect(position129, 108)
else
