...
function onSay(cid, words, param, channel)
        ...
        if(t[1] == "add") then
                ...
                db.query("INSERT INTO `auction_system` (`player`, `item_name`, `item_id`, `count`, `cost`, `date`) VALUES (" .. getPlayerGUID(cid) .. ", \"" .. t[2] .. "\", " .. getItemIdByName(t[2]) .. ", " .. itemcount .. ", " .. costgp ..", " .. os.time() .. ")")
                                doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "You successfully add " .. itemcount .." " .. t[2] .." for " .. costgp .. " JP ienes to offerts database.")
        end
        if(t[1] == "buy") then
                ...
                if(buy:getID() ~= -1) then
				local cost = buy:getDataInt("cost") * 10000
                        ...
                        doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "You bought " .. buy:getDataInt("count") .. " ".. buy:getDataString("item_name") .. " for " .. cost/10000 .. " JP Ienes!")
                        db.query("UPDATE `players` SET `auction_balance` = `auction_balance` + " .. cost .. " WHERE `id` = " .. buy:getDataInt("player") .. ";")
                        buy:free()
                else
                        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Wrong ID.")
                end
