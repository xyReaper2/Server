...
ChannelsList Chat::getCustomChannelList(Player* player, ChannelGroup customChannels)//add by matheus
{
	ChannelsList list;
	...
	for (ChannelGroup::iterator it = customChannels.begin(); it != customChannels.end(); ++it)
	{
		if (ChatChannel * newChannel = new ChatChannel(it->first, it->second, 0)) {
			list.push_back(std::make_pair(it->first, newChannel->getName()));
			/*
			CustomChannelMap::iterator find = m_customChannels.find(it->first);
			if (find == m_customChannels.end()) {
				m_customChannels.insert(std::pair<uint16_t, ChatChannel*>(it->first, newChannel));
			}*/
	...
	{
		if (ChatChannel * newChannel = new ChatChannel(i, "", 0)) {
			list.push_back(std::make_pair(newChannel->getId(), newChannel->getName()));
		}
	}
...
bool Chat::talkBlocked(Player* player, MessageClasses type, const std::string& text, uint16_t channelId,
	uint32_t statementId, bool anonymous/* = false*/)
{
		...
		|| (text[0] != '!' && text[0] != '/'))
	{
		...
		return channel->talkBlocked(player, type, text, statementId);
	}
...
bool ChatChannel::talkBlocked(Player* player, MessageClasses type, const std::string& text, uint32_t statementId)
{
	...
	player->sendCreatureChannelSay(player, type, text, m_id, statementId);
	/*
...
bool ChatChannel::talk(Player* player, MessageClasses type, const std::string& text, uint32_t statementId)
{
	...
	if (channelId == 5 || channelId == 8 || channelId == 9) {
		if (player->getGroupId() == 4 || player->getGroupId() == 5 || player->getGroupId() == 6) {
		ntype = MSG_GAMEMASTER_CHANNEL;
		}
		else if (player->getGroupId() == 2 || player->getGroupId() == 3) {
