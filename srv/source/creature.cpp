...
Creature::Creature()
{
    ...
    battleLock = invulnerable = immuneweapon = 0;
...
{
	return canSee(getPosition(), pos, 12, 11);
}
...
bool Creature::isBattleLock() const
{
	if(battleLock)
		return battleLock > (uint64_t)OTSYS_TIME();

	return false;
}

bool Creature::isInvulnerable() const
{
	if(invulnerable)
		return invulnerable > (uint64_t)OTSYS_TIME();

	return false;
}

bool Creature::isImmuneWeaponDamage() const
{
	if(immuneweapon)
		return immuneweapon > (uint64_t)OTSYS_TIME();
...
void Creature::onAttacking(uint32_t interval)
{
    ...
    if(getPlayer() && ((attackedCreature->getPlayer() && (getPlayer()->getSecureMode() == SECUREMODE_ON)) || isBattleLock() || attackedCreature->isBattleLock() || (attackedCreature->getMonster() && attackedCreature->getMonster()->isPlayerSagaBlock(this)))) { // Saga Sytem and BattleLock and optional pvp
	    setAttackedCreature(NULL);
	    setFollowCreature(NULL);
	    getPlayer()->sendCancelTarget();
	    return;
	} 
...
bool Creature::isStorageEqualValue(const uint32_t key, uint16_t value) const // Saga System
{
	std::stringstream k;
	k << key;
	std::stringstream v;
	v << value;
	StorageMap::const_iterator it = storageMap.find(k.str());
	if(it != storageMap.end())
	{	
		return (it->second !=  (v.str()));
	}
	return true;
}
	...
	bool checkDefense/* = false*/, bool checkArmor/* = false*/, bool/* reflect = true*/, bool/* field = false*/, bool/* element = false*/)
{
	...
	else if(checkDefense || checkArmor)
	{
		...
		{
			int32_t maxDefense = getDefense(), minDefense = maxDefense / 1;
		...
		if(checkArmor)
		{
			...
			if(damage <= 0)
			{
				...
				g_game.addMagicEffect(getPosition(), 68);
			}
		}
	...
	if(const MagicField* field = getTile()->getFieldItem())
	{
		switch(type)
		{
			...
			case CONDITION_SUITON:
				_remove = field->getCombatType() != COMBAT_SUITONDAMAGE;
				break;
			case CONDITION_FUUTON:
				_remove = field->getCombatType() != COMBAT_FUUTONDAMAGE;
				break;
			case CONDITION_DOTON:
				_remove = field->getCombatType() != COMBAT_DOTONDAMAGE;
				break;
			case CONDITION_RAITON:
				_remove = field->getCombatType() != COMBAT_RAITONDAMAGE;
				break;
			case CONDITION_MOKUTON:
				_remove = field->getCombatType() != COMBAT_MOKUTONDAMAGE;
				break;
			case CONDITION_SHOTON:
				_remove = field->getCombatType() != COMBAT_SHOTONDAMAGE;
				break;
			case CONDITION_SUNA:
				_remove = field->getCombatType() != COMBAT_SUNADAMAGE;
				break;
			case CONDITION_ENTON:
				_remove = field->getCombatType() != COMBAT_ENTONDAMAGE;
				break;	
			case CONDITION_HYOTON:
				_remove = field->getCombatType() != COMBAT_HYOTONDAMAGE;
				break;			
			case CONDITION_YOTON:
				_remove = field->getCombatType() != COMBAT_YOTONDAMAGE;
				break;			
			case CONDITION_RANTON:
				_remove = field->getCombatType() != COMBAT_RANTONDAMAGE;
				break;		
			case CONDITION_BAKUTON:
				_remove = field->getCombatType() != COMBAT_BAKUTONDAMAGE;
				break;			
			case CONDITION_SHAKUTON:
				_remove = field->getCombatType() != COMBAT_SHAKUTONDAMAGE;
				break;			
			case CONDITION_FUTTON:
				_remove = field->getCombatType() != COMBAT_FUTTONDAMAGE;
				break;			
			case CONDITION_JINTON:
				_remove = field->getCombatType() != COMBAT_JINTONDAMAGE;
				break;			
			case CONDITION_SATETSU:
				_remove = field->getCombatType() != COMBAT_SATETSUDAMAGE;
				break;			
			case CONDITION_TAIJUTSU:
				_remove = field->getCombatType() != COMBAT_TAIJUTSUDAMAGE;
				break;			
			case CONDITION_YIN:
				_remove = field->getCombatType() != COMBAT_YINDAMAGE;
				break;	
			case CONDITION_YANG:
				_remove = field->getCombatType() != COMBAT_YANGDAMAGE;
				break;
			case CONDITION_KENJUTSU:
				_remove = field->getCombatType() != COMBAT_KENJUTSUDAMAGE;
				break;
			default:
				break;
		}
