...
Account IOLoginData::loadAccount(uint32_t accountId, bool preLoad/* = false*/)
{
	...
	query << "SELECT `name`, `password`, `salt`, `premdays`, `lastday`, `key`, `warnings`, `lang_id` FROM `accounts` WHERE `id` = " << accountId << " LIMIT 1";
	DBResult_ptr result;
...
bool IOLoginData::loadAccount(Account& account, const std::string& name)
{
	...
	query << "SELECT `id`, `password`, `salt`, `premdays`, `lastday`, `key`, `warnings`, `lang_id` FROM `accounts` WHERE `name` " << db->getStringComparer() << db->escapeString(name) << " LIMIT 1";
	DBResult_ptr result;
	if(!(result = db->storeQuery(query.str())))
		return false;
...
void IOLoginData::loadCharacters(Account& account)
{
	...
	query << "SELECT `name`, `looktype` FROM `players` WHERE `account_id` = " << account.number << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " AND `deleted` = 0";
#else
	query << "SELECT `id`, `name`, `world_id`, `online`, `looktype` FROM `players` WHERE `account_id` = " << account.number << " AND `deleted` = 0";
#endif
	DBResult_ptr result;
	if(!(result = db->storeQuery(query.str())))
		return;
	...
	do
	{
		...
		account.charOutfitList.push_back(result->getDataInt("looktype"));
#else
		...
		if(GameServer* srv = GameServers::getInstance()->getServerById(result->getDataInt("world_id")))
			...
			account.charList[srv] = result->getDataInt("looktype");
		else
...
LocalizationLang_t IOLoginData::getAccountLanguage(uint32_t accountId)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `lang_id` FROM `accounts` WHERE `id` = " << accountId << " LIMIT 1";
	DBResult_ptr result;
	if(!(result = db->storeQuery(query.str())))
		return LANG_EN_US;
...
bool IOLoginData::setAccountLanguage(uint32_t accountId, LocalizationLang_t languageId)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "UPDATE `accounts` SET `lang_id` = " << languageId << " WHERE `id` = " << accountId << db->getUpdateLimiter();
...
bool IOLoginData::saveAccount(Account account)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "UPDATE `accounts` SET `premdays` = " << account.premiumDays << ", `warnings` = " << account.warnings << ", `lastday` = " << account.lastDay << ", `lang_id` = " << account.language << " WHERE `id` = " << account.number << db->getUpdateLimiter();
	return db->query(query.str());
}
...
Outfit_t IOLoginData::getPlayerOutfit(const std::string& name)
{
	...
	query << "SELECT `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons` FROM `players` WHERE `name` ";
	query << db->getStringComparer() << db->escapeString(name) << " AND `deleted` = 0 LIMIT 1";
...
uint32_t IOLoginData::getPlayerLevel(const std::string& name)
{    
    Database* db = Database::getInstance();
	DBQuery query;
	
	query << "SELECT `level` FROM `players` WHERE `name` ";
	query << db->getStringComparer() << db->escapeString(name) << " AND `deleted` = 0 LIMIT 1";
...
uint8_t IOLoginData::getPlayerVocation(const std::string& name)
{    
    Database* db = Database::getInstance();
	DBQuery query;
	
	query << "SELECT `vocation` FROM `players` WHERE `name` ";
	query << db->getStringComparer() << db->escapeString(name) << " AND `deleted` = 0 LIMIT 1";
	...
	return result->getDataInt("vocation");
}
