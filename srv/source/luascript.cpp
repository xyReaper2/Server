...
int32_t LuaInterface::luaDoPlayerSpectate(lua_State* L)
{
	...
	if (Player * player = env->getPlayerByUID(popNumber(L))) {
		...
		{
			player->sendTextMessage(MSG_INFO_DESCR, "Você precisa dar logout antes de poder usar o WATCH novamente!");
...
int32_t LuaInterface::luaDoCreatureSetNoMove(lua_State* L)
{
	...
	{
			creature->getPlayer()->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, "Você foi travado, não tente se mover.");
		}
		if (Player* player = creature->getPlayer())
		{
			player->sendCancelTarget();
...
int32_t LuaInterface::luaGetItemInfo(lua_State* L)
{
	...
	// Wonsr
	setField(L, "rareMode", item->rareSystemR);
	setField(L, "epicMode", item->rareSystemE);
	setField(L, "legendMode", item->rareSystemL);
