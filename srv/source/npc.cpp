	...
	for(xmlNodePtr q = node->children; q; q = q->next)
	{
		if(!xmlStrcmp(q->name, (const xmlChar*)"look"))
		{
			...
			if(readXMLInteger(q, "type", intValue))
			{
				...
				// if(readXMLString(q, "shader", strValue)) {
					// Shader* shader = Shaders::getInstance()->getShaderByName(strValue);
					// nType->outfit.lookShader = shader ? shader->clientId : 0;
				// }
			}
...
void Npc::doSay(const std::string& text, MessageClasses type, Player* player)
{
	...
	{
		std::string locText = Localization::t(player->getLanguage(), text);
        player->sendCreatureSay(this, type, locText);
		player->onCreatureSay(this, type, locText);
	}
}
