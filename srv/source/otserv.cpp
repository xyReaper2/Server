...
ServiceManager* services)
{
	...
	std::clog << ">> Loading localizations:" << std::endl;
	#if defined(WINDOWS) && !defined(_CONSOLE)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Loading localizations:");
	#endif
	if(!Localization::getInstance())
		startupErrorMessage("Unable to load localizations!");
